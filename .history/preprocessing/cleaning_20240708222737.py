...
df = df[['Furnished', 'Terrace', 'Garden', 'PropertyId',
...
preprocessor = ColumnTransformer(
    transformers=[
        ...
        ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), categorical_cols)  # Set sparse=False
    ])
...
def inverse_transform(encoded_df, original_df, categorical_cols):
    for col in categorical_cols:
        ...
        encoded_df[col] = encoded_df[one_hot_cols].idxmax(axis=1).apply(lambda x: x.split('_')[-1])
        # Drop the one-hot encoded columns
        encoded_df = encoded_df.drop(columns=one_hot_cols)
    return encoded_df

df_imputed = inverse_transform(df_imputed, df, categorical_cols)
print("\nDataFrame after reversing one-hot encoding:")
print(df_imputed.head(75))
