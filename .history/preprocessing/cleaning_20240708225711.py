...
df = df[['TypeOfProperty', 'PostalCode','Price', 'Kitchen', 'StateOfBuilding', 'Heating', 'Bedrooms', 'LivingArea', 'NumberOfFacades']]
...
def inverse_transform(encoded_df, original_df, categorical_cols):
    for col in categorical_cols:
        ...
        original_df[col] = encoded_df[one_hot_cols].idxmax(axis=1).apply(lambda x: x.split('__')[-1])  # Adjust split
